...
const float SHOT_COOLDOWN    = 0.20f;  // NEW: seconds between shots
const int   MAX_AMMO         = 20;     // NEW: ammo capacity
const float RELOAD_TIME      = 2.5f;   // NEW: reload duration (seconds)

// ── Enemy constants ─────────────────────────────────────────────
const float ENEMY_ATTACK_INT = 4.0f;   // seconds between laser attacks
const float ENEMY_DAMAGE     = 5.0f;   // damage per laser hit
...
const float HEAVY_DAMAGE = 10.0f;     // Heavy hits harder
